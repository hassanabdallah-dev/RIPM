...
databaseQuery = "insert into results_first_method_optimized_new (target_entity_code, target_entity_type, property, class, class_rank, Gini, proportion, objects_number, facts_number, Gini_times_proportion, Gini_Times_proportion_times_Rank, gini_calculation_method, time_to_get_result, sampling, queries_number, percentage, queries_number_per_record) VALUES "
entitiesPrefix = "http://www.wikidata.org/entity/"
propertiesPrefix = "http://www.wikidata.org/prop/direct/"
timePerOccupation = 0
time_per_thread = []
...
def process_property(property, ReferenceRelation, currentEntity, sparql):
    ...
    try:
        query = """SELECT ?class (COUNT(?s) AS ?count)
                    WHERE {
                      ?o wdt:""" + ReferenceRelation + """ wd:""" + currentEntity + """.
                      ?s wdt:""" + property + """ ?o.
                      ?s wdt:P31 ?class.
                    }
                    GROUP BY ?class
        ...
        for resultClass in resultsClasses["results"]["bindings"]:
            wikiClass = resultClass["class"]["value"][len(entitiesPrefix):]
            with lock:
...
def process_classes(property, Class, ReferenceRelation, currentEntity, sparql, rank, totalObjectsNumberPerEntity, entityTargetType, gini_calculation_method):
    ...
    try:
        ...
        with lock1:
            ...
            databaseQuery += "('" + str(currentEntity) + "', '"+entityTargetType+"', '" + str(property) + "', '" + str(Class) + "', " + str(rank) + ", " + str(GiniCoefficient) + ","+str(proportion)+","+str(ObjectCount)+","+str(FactCount)+" ," + str(GinitimesProportion) + ", "+str(GinitimesProportiontimesRank)+",'"+gini_calculation_method+"', " + "tempNull" + ", "+str(sampling)+", "+"QueriesTempNull"+", "+str(percentage)+", "+str(QueriesNumbersWithClasses[property][Class])+"),"
    ...
    for entityTargetType in ["classes"]:
        for gini_calculation_method in ["model", "data"]:
            ...
            if entityTargetType == "occupations":
                ReferenceRelation = "P106"
            elif entityTargetType == "classes":
                ReferenceRelation = "P31"
            ...
            if entityTargetType == "occupations":
                query = """	SELECT ?entity (COUNT(?o) AS ?count)
                            WHERE{
                              ?o wdt:"""+ReferenceRelation+""" ?entity  
                            }
                            group by ?entity
                            ORDER BY DESC(?count)
                            Limit 100"""
                ...
                for resultEntity in resultsEntities["results"]["bindings"]:
                    wikiEntity = resultEntity["entity"]["value"][len(entitiesPrefix):]
            ...
            list_of_entities = ["Q16521", "Q4164871", "Q5633421", "Q484170", "Q101352", "Q4830453", "Q215380", "Q4022", "Q476028", "Q7187", "Q34442", "Q21014462", "Q43229", "Q618779", "Q27020041", "Q34770", "Q747074", "Q3918"]
            #list_of_entities = ["Q13442814"]
            ...
            for currentEntity in list_of_entities:
                try:
                    ...
                    query = """SELECT (COUNT(*) AS ?count)
                                WHERE {
                                  ?o wdt:"""+ReferenceRelation+""" wd:"""+str(currentEntity)+""".
                                }"""
                    ...
                    if gini_calculation_method == "model":
                        ...
                        try:
                            ...
                            timePerOccupation += returnResult["time"]
                            queryNumber += 1
                        except Exception as sampex:
                            sampling = True
                            samplingError = 1
                            percentage = 90
                            #percentage = 1
                            if currentEntity == "Q1650915":
                                percentage = 15
                            while samplingError == 1:
                                try:
                                    print("Sampling: " + str(currentEntity) + " " + str(percentage))
                                    query = """SELECT ?property
                                                        (COUNT(DISTINCT ?o) AS ?distinctObjectCount)
                                                        (COUNT(*) AS ?totalFactCount)
                                                        WHERE {
                                                          ...
                                                          ?s ?property ?o.
                                                          FILTER(STRSTARTS(STR(?property), "http://www.wikidata.org/prop/direct/")).
                                                          FILTER (xsd:integer( SUBSTR( STR(?o) , STRLEN(STR(?o)) - 1) ) <= """+str(percentage)+""")
                                                        }
                                                        GROUP BY ?property
                                             """
                                    returnResult = sparqlQuery(query, sparql)
                                    ...
                                    samplingError = 0
                                    queryNumber += 1
                                except Exception as samplingExTwo:
                                    samplingError = 1
                                    percentage = percentage - 5
                                    if percentage <= 0:
                                        print("Sampling does not work: " + str(currentEntity))
                                        samplingDoNotWork = True
                                        break

                        if samplingDoNotWork == True:
                            print("Sampling does not work, continue: " + str(currentEntity))
                            continue

                        start_time = time.time()

                        for result in results["results"]["bindings"]:
                            propertyCode = result["property"]["value"][len(propertiesPrefix):]
                        ...
                        for result in results["results"]["bindings"]:
                            ...
                            thread = threading.Thread(target=calculate_gini_based_on_data, args=(ReferenceRelation, currentEntity, result["property"]["value"][len(propertiesPrefix):], sparqlInstance))
                            thread.start()
                    ...
                    try:
                        ...
                        with open("ErrorsWikidata.txt", "a", encoding="utf-8") as file:
