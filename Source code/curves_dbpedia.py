...
if __name__ == '__main__':
    ...
    ax2.scatter(modelValues, dataValues, color='Red', alpha=0.5)#, alpha=0.5
    ...
    ax2.set_title('(e) Gini coefficient computation time (seconds)', fontsize=14)
    ax2.set_xscale('log')
    ax2.set_yscale('log')
    ...
    ax1.scatter(modelValues, dataValues, color='Red', label="Classes", alpha=0.6)
    ...
    ax1.set_ylabel('Gini')
    ax1.set_title('(d) Gini coefficient values', fontsize=14)
    # plt.xscale('log')
    # plt.yscale('log')
    ...
    ax3.set_title('(f) Queries\' number', fontsize=14)
    ax3.set_xscale('log')
    ax3.set_yscale('log')
    ...
    ax3.grid(True)
    fig.suptitle('Experimentation made on Dbpedia', fontsize=16)
    fig.subplots_adjust(top=0.8)  # Adjust the value as needed
    plt.savefig('curves/new_gini_combined_dbpedia.pdf')
    plt.close()